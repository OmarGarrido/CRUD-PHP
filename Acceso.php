...
<html>
  <head>
    <title>Accesos de Estaciones</title>
  ...
  <body>
    ...
    <form name="buscar" method="post" action="Acceso.php" class="d-flex">
      <select name="campo">
        <?
    ...
    <div class="mt-3" align="center">
      <a class="btn btn-success" href="Acceso-insertar.php">Agregar</a></button>
    </div>
  </body>
</html>

<?
$sql="SELECT ID_ACCESO,NOM_ACCESO,ID_ESTACION3,NOM_ESTACION FROM ".$tabla." inner join ESTACION on ID_ESTACION3=ID_ESTACION";
...
$id=$_GET['borrar'];
    ...
    $sql="DELETE FROM ".$tabla." WHERE ID_ACCESO = '".$id."'";
    ...
    echo "<script>alert('Registro Eliminado');window.location='Acceso.php';</script>";
  }
  ?>
...
$id=$_GET['cambiar'];
    ...
    $sql = "SELECT ID_ACCESO,NOM_ACCESO,ID_ESTACION3,NOM_ESTACION FROM ".$tabla." inner join ESTACION on ID_ESTACION3=ID_ESTACION WHERE ID_ACCESO ='".$id."'";
    ...
    <div class="col-md-5 mx-auto mt-5">
      <div class="card card text-white bg-dark mb-3">
        <div class="card-body">
          ...
          <form action="<?php echo $_SERVER['PHP_self'];?>" method="post" name="Acceso.php">
          <p>
          <input  type="hidden" align="LEFT" name="ID" value="<?php echo $registro['ID_ACCESO'];?>" /><p>
          <p>Nombre de Acceso:
          <input class="form-control mt-2" type="text" align="LEFT" name="Nombre" value="<?php echo $registro['NOM_ACCESO'];?>"/><p>
          <p>ID Estacion:
          <input class="form-control mt-2" type="text" align="LEFT" name="Estacion" value="<?php echo $registro['ID_ESTACION3'];?>"/><p>         
          <input class="btn btn-success form-control mt-3" type="submit" value="Actualizar" name="actualizar">
          <a class="btn btn-danger form-control mt-3" href="Acceso.php">Atras</a>
          </form>
        </div>
      </div>
    </div>
...
$subs_estacion = utf8_decode($_POST['Estacion']);
    ...
    NOM_ACCESO='$subs_nombre',ID_ESTACION3='$subs_estacion' WHERE ID_ACCESO ='".$id."'";
    

    if(@mysql_query($sql)){
      echo '<script>alert("Registro Actualizado.");window.location="Acceso.php";</script>';
    }
    else{
...
if (isset($_POST['enviar'])) {
  $query ="SELECT ID_ACCESO,NOM_ACCESO,ID_ESTACION3,NOM_ESTACION from ".$tabla." inner join ESTACION on ID_ESTACION3=ID_ESTACION WHERE {$_POST['campo']} LIKE '%{$_POST['palabra']}%'";
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
 ...
 -->          
    ...
    <div class="mt-3" align="center">
      <a class="btn btn-danger" href="Acceso.php">Regresar</a></button>
    </div>
    <table id="texto" class="table table-dark table-striped table-hover mt-3" style="text-align: center; width: 800px; text-transform: capitalize;" align="center">
      <thead>
        <tr>
